...
export async function carregarQuestoes() {
  ...
  const resp = await fetch(`/certiacademy/course/AI-900/api/questions.php?simulado=${encodeURIComponent(simulado)}`);
...
function salvarResposta(q, selecionadas) {
    ...
    if (index > -1) {
        respostasUsuario[index] = { index: questaoAtual, selecionadas, pontos: pontosQuestao };
    } else {
        respostasUsuario.push({ index: questaoAtual, selecionadas, pontos: pontosQuestao });
    }
...
(async () => {
  ...
  // Restaura respostas do usuário, se houver
  const respostasSalvas = localStorage.getItem('respostasUsuario');
  if (respostasSalvas) {
    respostasUsuario = JSON.parse(respostasSalvas);
  }
