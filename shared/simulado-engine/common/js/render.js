import { enviarProblema } from './report.js';
...
function renderizarCombobox(questao, container, respostaSalva) {
  ...
  if (partesTexto.length !== 2) {
    ...
    questionText.textContent = questao.texto;
    container.appendChild(questionText);
    return;
  }

  const textoAntes = document.createElement("p");
  textoAntes.textContent = partesTexto[0].trim();
  questionText.appendChild(textoAntes);

  const select = document.createElement("select");
  select.id = "combo-resposta";
  ...
  questionText.appendChild(selectContainer);

  const textoDepois = document.createElement("p");
  textoDepois.textContent = partesTexto[1].trim();
  questionText.appendChild(textoDepois);
