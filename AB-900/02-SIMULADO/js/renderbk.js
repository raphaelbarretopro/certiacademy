import { respostasUsuario, questaoAtual, listaQuestoes, marcarQuestao, setQuestaoAtual, simuladoFinalizado } from './quiz.js';
...
export function renderizarLista(questoes) {
  ...
  questoes.forEach((_, i) => {
    ...
    if (respostasUsuario.some(r => r.index === i)) {
      bandeira.classList.add("marcada");
    }

    bandeira.onclick = (event) => {
      event.stopPropagation();
      marcarQuestao(i);
      renderizarLista(questoes);
    };

    li.appendChild(bandeira);
    li.appendChild(document.createTextNode(` Questão ${i + 1}`));

    li.onclick = () => {
      if (!respostasUsuario.find(r => r.index === questaoAtual) && !simuladoFinalizado) {
        const confirmar = confirm("Você ainda não confirmou esta resposta. Deseja sair mesmo assim?");
        if (!confirmar) return;
      }
      ...
      const respostaNovaSalva = respostasUsuario.find(r => r.index === i);
      if (respostaNovaSalva) {
        const q = questoes[i];
        import('./quiz.js').then(({ mostrarFeedback }) => {
          mostrarFeedback(q, respostaNovaSalva.selecionadas);
        });
      }
    };
